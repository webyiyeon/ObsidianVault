...
##### 모든 프로세스 종결(결재 승인까지)났는데도 '허가부서작성 - 결재중' 단계로 남아있는 현상
1. 현재 진행 단계 판정 parameter가 정상적으로 동작되지 않는 것으로 판단 
	- `appr_biz_cd` 로 판단하는데, 이 때 `SA-WO-04`로 가야하는 로직이 `SA-WO-03` 쪽으로 넘어가는 것으로 보임
2. 상세 데이터 호출 과정 추가 (getWkodMasterNew) 
3. 이후로도 추가 오류 발생
4. 호출된 데이터 로그에 남기는 print문 추가 후 재배포(25-09-09 17:33)
