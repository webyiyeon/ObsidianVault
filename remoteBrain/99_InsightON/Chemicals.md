...
==요청서 탭 ==
**CASE 1. 신규 등록(직접등록)**  
- [ ] 대상 구분 기준 선택 시 변경관리 대상 여부 자동체크 (1, 2 등급 = "N" / 3, 4등급 = "Y")  
- [x] 대상 구분 기준 선택 시 요청대상 자동 변경 (1, 2 등급 = "요청" / 3, 4등급 = "미요청")
- [ ] 요청대상 , 요청부서(팀), 단위공장명, 가동개시일, 점검일, 변경관리 대상 전부 필수값 표시 (\*)  
- [ ] 점검 분야 및 수신처의 공정,운전 대상자 필수값 (저장 시 비어있으면 선택하도록)  
- [ ] 필수값의 입력 여부 확인 및 Alert창 추가

**CASE 2.상세보기**  
- [ ] Detail 쿼리에 saf_chng_no 열 컬럼이 없다는 오류  
- [ ] 대상 구분 기준의 등급 변경 시 "선택 할 수 없습니다." 뜨고 체크 사라짐

**CASE 3. 저장 후 상세보기에서 결재요청**  
- [ ] 결재요청명 변경 필요 -> "가동 전 안전점검 요청서 결재요청"

==결과서 탭==  
**CASE 1. 요청서에서 넘어온 직 후(기안자 입장 - 요청대상 "Y")**  
- [ ] 승인요청 버튼은 전부 확인완료가 되었을 때 show  
- [ ] 점검대상 점검표의 공통사항은 무조건 "대상"으로 고정 (변경 못하도록)  
- [ ] 기기별 점검사항은 확인요청 하기 전에는 볼 수 만 있고, 점검결과 선택 / 지적사항 입력 못하도록 막기  
- [ ] 저장 시 바로 확인요청이 되는것이 아닌, 별도의 확인요청 버튼 분리
