...
- [ ] SHE절차서 선택 안되어있는데 SHENo 등 텍스트박스 보임  
- [ ] 사용자 검색 팝업 오픈 시 API호출 오류 발생 팝업  
- [ ] 변경관리 저장 시 Object.toString() 발생( 간헐적으로 발생 하는지는 모름)  
- [ ] 변경관리 저장 후 또 저장 시 관련자료 > 완료일자 유효성을 확인 하는데 관련자료의 유효성은 변경관리 위원장 승인 다음 단계에서만 확인 하면 될듯  (GSHE는 유효성 x)  
- [ ] 완료일자 입력 후 저장 시 Object.toString() 발생(저장 불가)