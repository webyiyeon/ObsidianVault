...
- [ ] **안전환경작업실시계획서 사전제출서류**
  - [ ] “공사업체이외 공사전 서명자료” 필수값 지정 (모두사인 활용)
  - [ ] “공사도급계약서 또는 공사발주서” 필수값 지정

- [ ] **작업안전분석목록 (JSA)**
  - [ ] JSA 조회 시 협력사 검색조건이 “완전 동일”로 설정되어 있음 → 수정 필요
  - [ ] JSA 작성 시 작업단계 목록 “엑셀 업로드 기능 오류” 수정
  - [ ] 협력사에서 JSA 제출 후, 결재 진행 중에도 수정 가능 → “재작성 요청” 없을 시 수정 불가하도록 변경

- [ ] **작업허가서**
  - [ ] 안전환경팀 검토의견이 기준정보와 다르게 일괄 동일 내용으로 삽입됨 → 개별 반영되도록 수정
  - [ ] 체크한 “안전조치사항”이 초기화되는 현상 해결
  - [ ] 작업위치가 결재 단계마다 초기화되는 현상 해결

- [ ] **출입자 교육**
  - [ ] 교육 이수증 PDF 파일이 간헐적으로 열리지 않음 → 정상 열람되도록 수정
  - [ ] 협력사 시스템에서 출입자 교육 명단 조회 오류 (다른 협력사 명단 보임 / 전혀 안 보임) 수정

- [ ] **환경분야**
  - [ ] [환경 > 환경통합법 > 운영일지 작성] 메뉴의 배출구 / 배출시설 / 대기방지시설 분류 기존 시스템과 동일하게 수정
...
- [ ] **안전환경작업실시계획서**
  - [ ] 결재 완료 시 “결재요청 / 회수 / 반려” 버튼 비활성화 (협력사 포함)
  - [ ] 사전제출서류 “약도” 첨부 후 사라지는 문제 해결

- [ ] **작업안전분석목록 (JSA)**
  - [ ] 수급인(협력업체) 검토 결과 입력란 추가 (현재 없음)
...
- [ ] **안전환경작업실시계획서**
  - [ ] “검토 내용 : 신청부서, 안전환경팀 의견” 입력해도 공란으로 보이는 문제 수정
  - [ ] 사전제출서류 다운로드 시 첨부 문서 전체가 한 번에 PDF 병합되어 내려받아지는 현상 개선 (개별 다운로드 가능하도록)

- [ ] **작업허가서 (편의사항 / 현장 의견)**
  - [ ] 신청부서 담당자 변경 기능 추가 (작업허가 발행 시)
  - [ ] “안전보호구 및 주요 필요 장구” 목록에 아래 항목 추가  
    - 내산앞치마  
    - 내산토시  
    - 방열앞치마  
    - 방열토시
