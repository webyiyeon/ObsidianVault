...
##### 우선순위 처리
\[우선순위 매우 높음]
- [ ] 작업허가서(작업현황)과 R프레도 연계 (매일 아침 작업현황 공유 메일)
- [ ] 출입자 교육 이수증 QR코드 인식 불가능
- [ ] 오류 메시지의 출력 개선(필수값을 기입하지 않거나, 입력 값을 잘못 넣었을 때 영어로된 긴 오류문의 출력되고 있음 → 정확한 현상을 오류 메시지로 출력한다면 현업 사용자들의 오류 신고 문의가 해소될 것으로 예상)


| [우선순위 높음]                                                                              |
| -------------------------------------------------------------------------------------- |
| 1. 안전환경작업실시계획서 사전제출서류 "공사업체이외 공사전 서명자료" 필수값 지정 (모두사인 활용)                               |
| 2. 안전환경작업실시계획서 사전제출서류 "공사도급계약서 또는 공사발주서" 필수값 지정                                        |
| 3. 작업안전분석목록에서 JSA 조회 시, 협력사의 검색조건이 "완전 동일"로 설정되어 있음                                    |
| 4. 작업안전분석목록 JSA 작성 시 작업단계 목록 "엑셀 업로드 기능 사용 오류"                                         |
| 5. 협력사에서 JSA를 제출한 뒤, 담당자의 "재작성 요청"을 받지 않은 상태에서도 자체적으로 수정할 수 있음 (결재 진행 중에도 내용 수정 시 반영됨) |
| 6. 작업허가서 안전환경팀 검토의견이 기준정보와 다르게 일괄 같은 내용으로 삽입됨                                          |
| 7. 작업허가서에서 체크한 "안전조치사항"이 초기화 되는 현상 다수 발생                                               |
| 8. 작업허가서에서 작업위치가 초기화되는 현상 (결재단계마다)                                                     |
| 9. 출입자 교육 이수증 PDF파일이 열리지 않음 (정말 가끔까다가 열림)                                              |
| 10. 협력사 시스템 출입자 교육 명단 조회 오류 (본인 협력사 외에도 다른 협력사의 교육 신청 내용을 조회할 수 있음, 또는 전혀 보이지 않음)      |
| 11. 환경분야 [환경>환경통합법>운영일지 작성]의 배출구, 배출시설, 대기방지시설의 분류 수정 필요 (이전 시스템과 동일하게)                |

| [우선순위 중간]                                                        |
| ---------------------------------------------------------------- |
| 1. 안전환경작업실시계획서 결재 완료 시 우측 상단 "결재요청, 회수, 반려" 버튼 삭제되도록 수정 (협력사 포함) |
| 2. 안전환경작업실시계획서 사전제출서류 "약도" 첨부해도 없어짐                              |
| 3. 작업안전분석목록 JSA 수급인(협력업체) 검토 결과 작성 불가 (입력칸이 아에없음)                |

| [우선순위 낮음]                                                                     |
| ----------------------------------------------------------------------------- |
| 1. 안전환경작업실시계획서 내 "검토 내용 : 신청부서, 안전환경팀 의견" 작성해도 공란으로 보임                        |
| 2. 안전환경작업실시계획서 사전제출서류 다운로드 시 첨부된 모든 문서 한 번에 다온로드됨 (PDF 병합됨)                   |
| 3. (편의 관련사항, 현장 의견) 작업허가 발행 시 신청부서의 담당자 변경 기능 필요                              |
| 4. (편의 관련사항, 현장 의견) 작업허가서 내 "안전보호구 및 주요 필요 장구"의 추가 [내산앞치마, 내산토시, 방열앞치마, 방열토시] |
