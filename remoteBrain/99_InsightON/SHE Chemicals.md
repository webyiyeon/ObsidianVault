...
- [x] 사용자 검색 팝업 오픈 시 API호출 오류 발생 팝업 _⇒ 공통/일시적 오류로 판단_
- [x] 변경관리 저장 후 또 저장 시 관련자료 > 완료일자 유효성을 확인 하는데 관련자료의 유효성은 변경관리 위원장 승인 다음 단계에서만 확인 하면 될듯 (GSHE는 유효성 x) _⇒ 일단 유효성 삭제_
- [x] 완료일자 입력 후 저장 시 Object.toString() 발생(저장 불가)
- [x] 변경관리 저장 시 Object.toString() 발생(간헐적으로 발생 하는지는 모름) _⇒ NullPointerException 추가 _
...
- [x] 요청서
  - [x] 결재 라인 (일자/시간 포함)
  - [x] 요청서\_\[안전점검명]\_날짜.pdf
- [ ] 기기별 점검사항
  - [ ] '구분' 항목 보이게 수정
  - [x] 기기별 점검사항\_\[안전점검명]\_날짜.pdf
- [ ] 결과서
  - [x] 결재 라인 보이지 않음(일자/시간 포함) (기존 절차서 내의 양식과 상이함)
  - [ ] 출력물의 양식 Table에서 박스 양옆 선 제거 가능 여부 검토 _⇒ 검토니까 다음에_
  ...
  - [x] 결과보고서\_\[안전점검명]\_날짜.pdf
- [ ] 승인서
  - [x] 결재의 일자/시간 등이 출력되지 않음
  - [ ] 판정결과 양식이 출력물이 절차와 시스템과 일치하지 않음 (최종판정(가동 승인, 가동~))
  - [ ] 부적합 건수 취합 테이블 상이
  - [x] 미완료 개선항목 출력안됨 오류 수정
  - [ ] 미완료 개선항목과 승인판정결과 순서 바뀜(출력 순서) _⇒ 바꾸면 sub report 이슈 있으니까 다음에 바꾸기_
  - [x] 승인서\_\[안전점검명]\_날짜.pdf
...
##### 협력사 계정 대상으로 비밀번호 암호화 강화

- 로그인 시
  1. Crypto (기존방식)으로 1차 체크 (임직원 계정일 경우 기존 keep)
  2. Partner 계정인 경우, SHA-512 복호화 체크
- 비밀번호 변경/초기화 시 SHA-512로 암호화 처리

#### UGAC 권한 동기화

1. AuthGrpService.java 에 있는 getInitUgac로 API 전송 (추가만 함 변경 X)
2. LoginService에서 로그인 성공 시점에 UGAC 동기화 로직 서비스 호출
   - 이 때, AsyncUgacService에서는 비동기로 동작 진행하며,
     ushe 권한 정보 + ugac 권한 정보를 비교하여
     동일한 경우 pass / 다를 경우 ugac기준으로 동기화 처리
...
1. 취급화학물질 **기타** 선택 후 Key-in 입력 시 
   안전조치사항 MSDS 체크는 되지만 결재단계 넘어가면서 Key-in 항목 사라짐
   출력물에 표기 안됨
   MSDS 첨부 팝업 뜨지 않음
	![[Pasted image 20260423174334.png|800]]
	![[Pasted image 20260423174342.png]]
2. 출력물 > 첨부파일 항목 중 안전작업실시계획서 부분 자체작업일 경우 미첨부, 업체 작업일 경우 첨부 표시되어야함
	![[Pasted image 20260423174350.png]]
3. 파일 업로드 + 삭제 + 결재 진행 시 파일은 삭제되지만 Validation 통과 > 첨부파일 팝업 뜨지 않음
4. Blind, LOTO, P&ID 미첨부 사유 입력 시 출력물 > 검토의견에 내용 기입되도록 ⇒ 킥 오프 미팅에서 논의되었던 사항이라고 함(w/정의범M, 이세진M)
