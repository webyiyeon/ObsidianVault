...
- [ ] Blind 설치 해체 작업은 일반 작업에도 있고 안전 작업에도 있다.  
- [ ] 전기, 밀폐 공간은 안전 작업에 속한다.  
- [ ] 기압 Test와 탱크로리 입출하 작업은 일반 작업에 속한다.  
- [ ] 탱크로리 입출하 작업은 첨부를 발생시키는 조건이 아니라, 취급 화학물질과 함께 선택되었을 때 공통 첨부 체크를 해제하는 조건이다.  
- [ ] 전기 작업은 다른 조건보다 우선 적용된다.
...
- [ ] 위 조건 중 하나라도 선택되면 다음 세 항목이 자동 체크된다.
	- [ ] LOTO,Blind ,P&ID List 첨부  

- [ ] 위 조건이 모두 해제되면 위 세 항목도 자동 해제되어야 한다.

- [ ] 일반위험작업에서 유해위험물질 저장, 취급, 사용 선택이 되어있는 경우 기압 Test, Blind 설치 해제 작업 체크 해제 시 자동으로 안전조치사항 > 공통 > LOTO List, Blind List, P&ID 첨부가 해제되지 않음
...
- [ ] 허가부서 단계에서는 Blind 설치 해체 작업 선택 여부를 기준으로 필수 첨부를 판단한다.
...
- [ ] 예를 들어 전기와 밀폐 공간이 함께 선택된 경우에는 공통 첨부 세 항목이 자동 체크되고, 신청부서에서는 LOTO List와 전기 단선도가 필수이며, 허가부서에서는 Blind 작업이 아니므로 LOTO List와 P&ID가 필수이다.

- [ ] 전기와 Blind 설치 해체 작업이 함께 선택된 경우에는 공통 첨부 세 항목이 자동 체크되고, 신청부서에서는 LOTO List와 전기 단선도가 필수이며, 허가부서에서는 Blind List와 P&ID가 필수이다.
...
- [ ] IV(초기화 벡터) 하드코딩
	→ 호출 옵션 속성 `false`로 변경. 해당 코드 삭제(HTTPS 사용)
- [ ] 권한 체크 미흡으로 인해 판교 본사, 울산 등 전체 파일 노출
- [ ] 암호화 또는 DRM 미적용으로 인한 기밀 정보 대량 유출 문제
