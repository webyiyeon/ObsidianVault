...
[[2024-04-25 Thu]]
	[공유] Elasticsearch 진행상황
	<작업 완료>
	- 141 ERP mssql + 141 쇼핑몰 mysql 데이터 join 후 삽입 프로세스 
	- 상품 인덱스 주기적 update / delete
	- 141 쇼핑몰의 레시피 데이터 인덱스 삽입
	- 쇼핑몰 web 검색 결과에 레시피 출력 → ex_레시피 컨텐츠에 있는 키워드가 검색 시 함께 반영되어 '사이다' 검색 시 연관 레시피인 '화채' 출력
	<작업 중>
	- 레시피 인덱스 주기적 update / delete 
	- 쇼핑몰 web 레시피 인덱스 반영 