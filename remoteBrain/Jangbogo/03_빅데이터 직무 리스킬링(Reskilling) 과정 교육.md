...
- Stable Diffusion: 이미지를 Latent Space에 투영하고 다시 복원(Diffuse, 확산)하면서 새로운 이미지 생성
...
- ChatGPT, Stable Diffusion과 같은 초거대 생성형 AI의 등장으로 데이터 저작권에 대한 이슈 발생
	1. AI의 학습에 인간의 창작물을 동의없이 학습하는 것은 저작권 침해에 해당한다.
	2. AI의 학습은 인간의 훈련과정과 다를 것 없이 진행되기 때문에 저작권 침해에 해당되지 않는다.
- 해당 논쟁으로 인해 전세계적으로 논의 중이며, 미국 등의 특정 국가에서는 대국민 설문을 진행하기도 함
- AI 규제 vs 자유의 범위 결정은 Trade-off 관계에 있으며, 한 쪽 입장의 극단적인 방향은 큰 부작용을 초래할 수 있음 
...
- 프롬프트 엔지니어링은 인공지능 언어 모델을 효과적으로 활용하기 위해 모델에게 적절한 질문과 명령을 제공하는 과정
- 인공지능 언어 모델에 정보를 요청하거나 작업을 수행하도록 하는 질문이나 명령을 작성
- 명확한 결과를 얻기 위해 구체적이고 명료한 프롬프트를 사용하는 것이 중요
- 응답 품질을 평가하여 모델이 효과적으로 작동하는지 확인
- 프롬프트를 수정하거나 개선하여 언어 모델의 성능을 최적화하는 과정을 활용 


#### 프롬프트 엔지니어링의 발전 과정

1. 생성 AI의 발전, ChatGPT(텍스트 생성 AI), Stable Diffusion(그림 생성)의 등장
2. 효율적인 AI 모델 활용의 중요성, 높은 기능 수준을 갖춘 AI가 개발되고 현실 생활 및 직무에서 어떻게 효율적으로 활용할 지 주목받고 있음
3. 다양한 응용 분야, 현재는 텍스트 및 그림 생성 AI를 가장 많이 사용하지만 최근에 영상, 음악 등의 타 분야에도 해당 기술이 적용되고 있음
4. 생성 AI의 활용 영역이 확장됨에 따라서, 프롬프트 엔지니어링의 유형이 다양해지고 중요성 또한 커지고 있음


#### 프로세스

1. 적절한 형태의 AI 모델 입력값 생성
2. AI 모델이 결과를 잘 도출하는지 확인
3. 완성된 프롬프트를 기반으로 안정적인 출력 성능을 확인할 수 있도록 검증 수행
	- 100건의 테스트 샘플을 가지고 정해진 프롬프트를 통해서 얼마나 정확하게 의도한 결과를 출력하는지 테스트


#### 프롬프트 엔지니어링 방법론

ChatGPT에 질문(프롬프트)을 작성할 때도 어느 정도의 규칙이 존재함

1. 명확하고 구체적인 요구사항 제시
2. 구어적인 표현보다 명시적으로 프롬프트 작성
3. 필요시 문맥과 배경 정보 제공
4. 페르소나(가상 역할) 설정을 통한 프롬프트 사용
5. 프롬프트 단계의 세분화 및 다른 표현 사용
6. 원하는 응답의 길이를 정성적 및 정량적으로 제공

특히, 애매모호한 지시보다 구체적이고 세분화된 지시가 효과적일 때가 많음

두 가지 측면의 접근 방식이 있음 

1. 실제 ChatGPT와 같은 LLM을 가지고 다양한 프롬프트 방식에 대한 답변 정확도 검증
2. 사용자들이 경험적으로 공유해놓은 템플릿 활용(뤼튼 등)

최신 동향으로는 LLM이 발전하면서 프롬프트의 작성 방식을 엔지니어링 하기 보다, 
다양한 유형에 대한 작업을 구조화해놓은 예시를 공유하고 연구하는 방향으로 발전하고 있음
...
- OpenAI의 공식 서비스로, 다양한 유형의 텍스트를 대화형으로 생성할 수 있음
- 이와 같은 유형의 모델을 LLM(Large Language Model)이라고 주로 정의함
- ChatGPT의 등장과 함께 LLAMA, SOLAR, Claude3 등 다양한 LLM들이 기술 수준을 발전시키고 있음
...
- ChatGPT, Gemini, Llama와 같은 모델을 LLM(Large Language Model)이라고 분류함
- LLM은 대규모 언어 모델을 의미하며, 대용량 텍스트를 사전학습하여 여러 분야에 적용
- NLU와 NLG 분야가 있는데, 최근에는 GPT 기반의 NLG가 주목 받고 있음
- 특히, ChatGPT의 등장으로 AI 분야에 혁신적인 가치를 창출하게 됨 
...
- LLM 기반 어플리케이션 개발을 위한 프레임워크, 다양한 LLM 관련 기능들을 결합(체인)하여 손쉽게 사용하도록 지원
- 핵심 기능 2가지
